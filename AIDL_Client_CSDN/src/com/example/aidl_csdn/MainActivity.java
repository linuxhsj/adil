 ...
 * */
	public void foo() {
		...
		try {
			...
			boolean login=true;
			UserData userData=iBase.foo(login);

			Toast.makeText(getApplicationContext(), "getUserData服务端返回内容："+userData.getMsg().toString()+ " and " +userData.isLogin() , Toast.LENGTH_SHORT).show();
	...
	public void foo2() {
		...
		try {
			...
			boolean backBoolean =result[result.length-1]!= 0;
			Toast.makeText(getApplicationContext(), "发送boolean："+sentBoolean+ " 返回 " +Arrays.toString(result) +" boolean:"+backBoolean, Toast.LENGTH_SHORT).show();
		} catch (RemoteException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
